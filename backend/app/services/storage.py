...
def get_cached_analysis(youtube_url: str) -> AnalysisResult | None:
    ...
    cursor.execute(
        "SELECT key, chords, audio_path FROM analyses WHERE url_hash = ?",
        (url_hash,)
    )
    row = cursor.fetchone()
    conn.close()
    ...
    key, chords_json, audio_path = row
    from app.models.analysis import Chord
    chords = [Chord(**chord) for chord in json.loads(chords_json)]
    
    return AnalysisResult(key=key, chords=chords, audio_path=audio_path)
...
def save_analysis(youtube_url: str, result: AnalysisResult) -> int:
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO analyses (youtube_url, url_hash, key, chords, audio_path)
            VALUES (?, ?, ?, ?, ?)
        """, (youtube_url, url_hash, result.key, chords_json, result.audio_path))
        ...
        cursor.execute("""
            UPDATE analyses SET key = ?, chords = ?, audio_path = ?
            WHERE url_hash = ?
        """, (result.key, chords_json, result.audio_path, url_hash))
...
def get_analysis(record_id: int) -> AnalysisResult | None:
    ...
    cursor.execute("SELECT key, chords, audio_path FROM analyses WHERE id = ?", (record_id,))
    row = cursor.fetchone()
    conn.close()
